...
const Feed = () => {
  ...
  const [isPrivate, setIsPrivate] = useState(false); // Naya state: Privacy ke liye
  ...
  const onPostSubmit = async e => {
    e.preventDefault();
    try {
      // Post karte waqt isPrivate ki value bhi bhejenge
      await api.post('/posts', { text, isPrivate });
      getPosts();
      setText('');
      setIsPrivate(false); // Form ko reset karo
    } catch (err) { console.error(err); }
  };
            ...
            <div key={post._id} className="card shadow-sm mb-3">
              <div className="card-body">
                <div className="d-flex justify-content-between">
                  ...
                  {/* Agar post private hai toh badge dikhao */}
